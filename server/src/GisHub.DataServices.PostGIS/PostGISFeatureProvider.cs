...
public class PostGISFeatureProvider : FeatureProvider {
    ...
    protected virtual string BuildMvtSql(DataServiceCacheItem layer, int z, int y, int x) {
        ...
        sqlBuilder.AppendLine($"    {string.Join(',', layer.Fields.Where(f => f.Name != layer.GeometryColumn).Select(f => f.Name))}, {layer.PrimaryKeyColumn} as fid_{layer.PrimaryKeyColumn}");
        ...
        sqlBuilder.AppendLine($"select ST_AsMVT(mvt_geom, '{layer.DataServiceName}', 4096, '{layer.GeometryColumn}', 'fid_{layer.PrimaryKeyColumn}')");
